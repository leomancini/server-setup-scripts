...
if pm2 delete "$SERVICE_ID" > /dev/null 2>&1; then
    echo -e "${BOLD_GREEN}SUCCESS${END_COLOR} Removed $SERVICE_ID from PM2"
else
...
config.apps = config.apps.filter(app => app.name !== '$SERVICE_ID');
