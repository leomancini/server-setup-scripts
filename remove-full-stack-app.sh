...
if pm2 delete "$APP_ID" > /dev/null 2>&1; then
    echo -e "${BOLD_GREEN}SUCCESS${END_COLOR} Removed $APP_ID from PM2"
else
    echo -e "${BOLD_RED}FAILED${END_COLOR} Cannot remove $APP_ID from PM2"
fi

# Remove app from ecosystem.config.cjs
ECOSYSTEM_FILE="/home/$USER/ecosystem.config.cjs"
if node -e "
const fs = require('fs');
const config = require('$ECOSYSTEM_FILE');
config.apps = config.apps.filter(app => app.name !== '$APP_ID');
...
"; then
    echo -e "${BOLD_GREEN}SUCCESS${END_COLOR} Removed $APP_ID from ecosystem.config.cjs"
else
    echo -e "${BOLD_RED}FAILED${END_COLOR} Cannot remove $APP_ID from ecosystem.config.cjs"
fi
