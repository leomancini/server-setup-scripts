...
if [[ "$GIT_SETUP" =~ ^([yY][eE][sS]|[yY])+$ ]]; then
	...
	if echo "#!/bin/bash
echo -e \"${BOLD_GREEN}SUCCESS${END_COLOR} Deployed main to $DOMAINS_DIRECTORY/$DOMAIN_NAME/html\"
git --work-tree=$DOMAINS_DIRECTORY/$DOMAIN_NAME/html --git-dir=$DOMAINS_DIRECTORY/$DOMAIN_NAME/$DOMAIN_NAME.git checkout main -f" | sudo tee $DOMAINS_DIRECTORY/$DOMAIN_NAME/$DOMAIN_NAME.git/hooks/post-receive > /dev/null; then
